...
@Service("ghfService")
public class GHfServiceImpl implements GHfService {
	@Autowired
	private JfDAO jfDAO;
	@Autowired
	...
	public void saveBill(JfForm form,UserBase currentUser,ServletContext servletContext) throws RuntimeException {
		...
		PaymentFixedline paymentFixedline = null;
		...
		if (payAccountBill == null) {
			...
			paymentFixedline = new PaymentFixedline(null, form.getUserId(), 0l,
					DateUtil.convertStringToDate("yyyy-MM-dd HH:mm:ss", begin),
					DateUtil.convertStringToDate("yyyy-MM-dd HH:mm:ss", end),
					form.getPeriodFrequency(), 0,form.getBillMoney(), form
			...
			BillModel bm = new BillModel(null,currentUser.getId(),"固话",form.getPayType().toString(),form.getEntId());
			jfDAO.saveBill(payAccountBill,paymentBill,paymentFixedline,bm);
			...
			//存账单源数据值
			paymentFixedline = (PaymentFixedline) jfDAO.get(PaymentFixedline.class,paymentBill.getId());
			jfDAO.saveBillSubjectData(form,paymentBill,payAccountBill,paymentFixedline);
