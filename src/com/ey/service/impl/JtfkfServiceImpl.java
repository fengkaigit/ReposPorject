...
public class JtfkfServiceImpl implements JtfkfService {
	...
	public void saveBill(JfForm form,UserBase currentUser,ServletContext servletContext) throws RuntimeException {
		...
		if (payAccountBill == null) {
			...
			BillModel bm = new BillModel(null,currentUser.getId(),"交通",form.getPayType().toString(),form.getEntId());
			jfDAO.saveBill(payAccountBill, paymentBill, paymentTraffic,bm);
			...
			//存账单源数据值
			paymentTraffic = (PaymentTraffic) jfDAO.get(PaymentTraffic.class,paymentBill.getId());
			jfDAO.saveBillSubjectData(form,paymentBill,payAccountBill,paymentTraffic);
			...
			if (count.longValue() == 0) {
				setting = new PaymentSetting(null, form.getUserId(), 0, "自家",
						...
						form.getPayType(), null, form.getEntId(), form
								...
								form.getVehicleNumber(),form.getCarframeNumber(),form.getEngineNumber());
