...
public class CnfServiceImpl implements CnfService {
	...
	public void saveBill(JfForm form) throws RuntimeException {
		...
		PaymentHeating paymentHeating = null;
		...
		if (payAccountBill == null) {
			...
			paymentBill = new PaymentBill(null, null, form.getUserId(), 0,
					...
					form.getBusinessType(), form.getPaymentStatus(), form
							.getPaymentMode(), UUIdUtil.getUUId(), form
							.getDivideStatus(), form.getAreaId(), form
							.getAreaName(), form.getAgentId(), form
							.getAgentName(), form.getBillNo(),
					form.getRemark(), form.getPayAddress(),form.getYear(),new Integer(form.getMonth()));
			String begin = form.getYear() + "-" + form.getMonth()
					+ "-01 00:00:01";
			String end = form.getYear()
					+ "-"
					+ form.getMonth()
					+ "-"
					+ DateUtil.getLastDayOfMonth(form.getYear(), new Integer(
							form.getMonth())) + " 23:59:59";
			paymentHeating = new PaymentHeating(null, form.getUserId(), 0l,
